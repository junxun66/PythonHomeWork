...
NIKE_HOME_URL ="https://www.nike.com/cn/" #"https://www.nike.com/us/en_us/"
...
def login(driver, username, password):
   ...
   # wait_until_clickable(driver=driver, xpath="//li[@js-hook='exp-join-login']/button")
    while True:
        try:
         wait_until_clickable(driver=driver, xpath="//div[@id='AccountNavigationContainer']/button", duration=2)
         break
        except TimeoutException:
            driver.refresh()
    LOGGER.info("Clicking login button")
    driver.find_element_by_xpath("//div[@id='AccountNavigationContainer']/button").click()

    LOGGER.info("Waiting for login fields to become visible")
    #wait_until_visible(driver=driver, xpath="//input[@name='emailAddress']")
    wait_until_visible(driver=driver, xpath="//input[@name='verifyMobileNumber']")
    LOGGER.info("Entering username and password")
    email_input = driver.find_element_by_xpath("//input[@name='verifyMobileNumber']")
    email_input.clear()
    ...
    #wait_until_visible(driver=driver, xpath="//a[text()='我的帐户']")
    wait_until_visible(driver=driver, xpath="//span[text()='我的帐户']")
...
def select_shoe_size(driver, shoe_size):
    ...
    wait_until_visible(driver, xpath="//div[@name='skuAndSize']", duration=10)

    LOGGER.info("Selecting size from dropdown")
    #driver.find_element_by_class_name("skuAndSize").find_element_by_xpath(
     #   "//label[text()='{}']".format(shoe_size)).click()
    driver.find_element_by_xpath("//div[@name='skuAndSize']").find_element_by_xpath(
    "//label[text()='{}']".format(shoe_size)).click()

def click_buy_button(driver):
    xpath = "//button[contains(@class,'addToCartBtn')]"#"//button[@data-qa='feed-buy-cta']"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--page-load-timeout", type=int, default=15)
